...
class GerritMonitor:
        ...
        if 'emails' in self.config:
            ...
            if override_check_days is not None:
                for email_config in self.emails:
                    email_config['check_days'] = override_check_days
        else:
            self.emails = []
    ...
    def fetch_changes_for_email(self, email_config: Dict) -> List[Dict]:
        """Fetch changes from Gerrit API for a specific email owner."""
        ...
        # Build Gerrit API query for owner email
        # Query format: owner:EMAIL after:DATE
        query = f"owner:{email} after:{start_date_str}"
        ...
        try:
            logger.info(f"Fetching changes from Gerrit for email: {email}")
            response = requests.get(api_url, params=params, timeout=30)
            response.raise_for_status()
            ...
            changes = json.loads(content)
            logger.info(f"Found {len(changes)} changes for {email}")
            return changes
            
        except requests.exceptions.RequestException as e:
            logger.error(f"Error fetching changes from Gerrit for {email}: {e}")
            return []
        except json.JSONDecodeError as e:
            logger.error(f"Error parsing Gerrit response for {email}: {e}")
            return []
    ...
    def fetch_all_email_changes(self) -> Dict[str, List[Dict]]:
        """Fetch changes for all configured emails."""
        all_email_changes = {}
        for email_config in self.emails:
            email = email_config['email']
            changes = self.fetch_changes_for_email(email_config)
            all_email_changes[email] = changes
        return all_email_changes
    
    def categorize_changes(self, changes: List[Dict], include_project: bool = False) -> Dict[str, List[Dict]]:
        ...
        for change in changes:
            ...
            # Include project name if requested (for email-based reports)
            if include_project:
                change_info['project'] = change.get('project', 'Unknown')
...
class ReportGenerator:
    ...
    def generate_email_markdown_report(self, all_categorized_changes: Dict[str, Dict[str, List[Dict]]],
                                       emails_config: List[Dict], gerrit_url: str) -> str:
        """Generate a markdown report of the changes for multiple email owners."""
        # Get current date for report
        report_date = datetime.utcnow().strftime('%Y-%m-%d %H:%M:%S UTC')
        
        # Calculate total changes across all emails
        total_changes = 0
        for categorized in all_categorized_changes.values():
            total_changes += sum(len(changes) for changes in categorized.values())
        
        report = []
        report.append("# 📧 Gerrit Activity Report by Owner\n")
        report.append(f"**Generated:** {report_date}\n")
        
        # Show different format based on number of emails
        if len(emails_config) > 1:
            # Multiple emails: show "Owners:" and "Total Changes:"
            email_addresses = ', '.join([e['email'] for e in emails_config])
            report.append(f"**Owners:** {email_addresses}\n")
            report.append(f"**Total Changes:** {total_changes}\n")
        else:
            # Single email: show "Owner:" and "Changes:" (without "Total")
            email_address = emails_config[0]['email']
            report.append(f"**Owner:** {email_address}\n")
            report.append(f"**Changes:** {total_changes}\n")
        
        report.append("\n---\n")
        
        # Generate report for each email
        for email_config in emails_config:
            email = email_config['email']
            days = email_config.get('check_days', 7)
            
            if email not in all_categorized_changes:
                continue
                
            categorized_changes = all_categorized_changes[email]
            email_total = sum(len(changes) for changes in categorized_changes.values())
            
            start_date = (datetime.utcnow() - timedelta(days=days)).strftime('%Y-%m-%d')
            end_date = datetime.utcnow().strftime('%Y-%m-%d')
            
            # Email header - use "Owner" label
            report.append(f"\n## Owner: {email}\n")
            day_label = "day" if days == 1 else "days"
            report.append(f"**Period:** {start_date} to {end_date} ({days} {day_label})\n")
            report.append(f"**Changes:** {email_total}\n")
            report.append("\n")
            ...
            if categorized_changes['merged']:
                ...
                for change in categorized_changes['merged']:
                    ...
                    report.append(f"- **Change #:** {change['number']}\n")
                    report.append(f"- **Author:** {change['owner']}\n")
                    report.append(f"- **Project:** {change.get('project', 'Unknown')}\n")
                    report.append(f"- **Changes:** +{change['insertions']} / -{change['deletions']} lines\n")
                    report.append(f"- **Updated:** {change['updated']}\n")
...
class SlackNotifier:
    ...
    def format_single_email_message(self, email: str, categorized_changes: Dict[str, List[Dict]],
                                     days: int, email_index: int = 0, total_emails: int = 1) -> Dict:
        """Format a single email's changes into a Slack message."""
        email_total = sum(len(changes) for changes in categorized_changes.values())
        
        # Build message blocks for single email
        blocks = [
            {
                "type": "header",
                "text": {
                    "type": "plain_text",
                    "text": f"📧 Gerrit Activity Report by Owner",
                    "emoji": True
                }
            },
            {
        ...
        # Add email counter if multiple emails
        if total_emails > 1:
            blocks.append({
                "type": "context",
                "elements": [{
                    "type": "mrkdwn",
                    "text": f"Owner {email_index + 1} of {total_emails}"
                }]
            })
        ...
        if categorized_changes['merged']:
            ...
            for change in categorized_changes['merged'][:10]:
                project_info = f" [{change.get('project', 'Unknown')}]" if 'project' in change else ""
                blocks.append({
                    "type": "section",
                    "text": {
                        "type": "mrkdwn",
                        "text": f"• <{change['url']}|#{change['number']}>: {change['subject']}{project_info}\n  (+{change['insertions']}/-{change['deletions']})"
                    }
                })
...
class EmailNotifier:
    ...
    def markdown_to_html(self, markdown_content: str) -> str:
        ...
        for line in lines:
            ...
            if in_header:
                if line.startswith('**Total Changes:**'):
                    ...
                    # Multiple owners - use owners instead of projects
                    owners_match = re.search(r'\*\*Owners:\*\* (.+)', line)
                    if owners_match:
                        projects = [p.strip() for p in owners_match.group(1).split(',')]
                elif line.startswith('**Owner:**'):
                    # Single owner - use owner instead of project
                    owner_match = re.search(r'\*\*Owner:\*\* (.+)', line)
                    if owner_match:
                        projects = [owner_match.group(1)]
                        is_single_project = True
            elif line.startswith('## Project:'):
        ...
        # Detect if this is an owner-based report by checking the markdown content
        is_owner_report = '# 📧 Gerrit Activity Report by Owner' in markdown_content
        
        # Format projects/owners display
        if is_single_project or len(projects) == 1:
            ...
            # Multiple projects/owners format
            if is_owner_report:
                project_label = "Owners"
            else:
...
def main():
    ...
    # Validate that at least one monitoring target is configured
    if not monitor.projects and not monitor.emails:
        logger.error("❌ No monitoring targets configured!")
        logger.error("   Please configure at least one project or email in config.json")
        logger.error("   Example:")
        logger.error('   "projects": [{"name": "project/name", "check_days": 1, "max_results": 100}]')
        logger.error('   OR')
        logger.error('   "emails": [{"email": "user@example.com", "check_days": 1, "max_results": 100}]')
        sys.exit(1)
    ...
    # Fetch and categorize changes for all emails (if configured)
    all_email_categorized = {}
    if monitor.emails:
        logger.info(f"\nMonitoring {len(monitor.emails)} email(s):")
        for email_config in monitor.emails:
            email = email_config['email']
            check_days = email_config.get('check_days', 7)
            logger.info(f"  - {email} (last {check_days} days)")
        
        all_email_changes = monitor.fetch_all_email_changes()
        
        for email, changes in all_email_changes.items():
            categorized = monitor.categorize_changes(changes, include_project=True)
            all_email_categorized[email] = categorized
            
            email_total = sum(len(c) for c in categorized.values())
            logger.info(f"Email {email}: {email_total} changes found")
    
    # Generate markdown report (only if projects are configured)
    ...
    if monitor.emails and all_email_categorized:
        email_markdown_report = report_generator.generate_email_markdown_report(
            all_email_categorized,
            monitor.emails,
            monitor.gerrit_url
        )
    ...
    if slack_webhook_url:
        ...
        # Send separate message for each email to avoid size limits
        if monitor.emails and all_email_categorized:
            total_emails = len(monitor.emails)
            email_slack_success_count = 0
            email_slack_fail_count = 0
            
            for idx, email_config in enumerate(monitor.emails):
                email = email_config['email']
                days = email_config.get('check_days', 7)
                
                if email in all_email_categorized:
                    logger.info(f"Sending Slack notification for email {email} ({idx + 1}/{total_emails})...")
                    message = notifier.format_single_email_message(
                        email,
                        all_email_categorized[email],
                        days,
                        idx,
                        total_emails
                    )
                    ...
                    if notifier.send_message(message):
                        email_slack_success_count += 1
                    else:
                        email_slack_fail_count += 1
            
            if email_slack_success_count == total_emails:
                logger.info(f"✅ All {total_emails} email-based Slack notifications sent successfully!")
            elif email_slack_success_count > 0:
                logger.warning(f"⚠️  Sent {email_slack_success_count}/{total_emails} email-based Slack notifications ({email_slack_fail_count} failed)")
            else:
                logger.warning("⚠️  Failed to send email-based Slack notifications (report still saved)")
    else:
    ...
    if all([smtp_host, smtp_user, smtp_password, from_email, to_emails_str]):
        ...
        # Send project report email if projects are configured
        report_date = datetime.utcnow().strftime('%Y-%m-%d')
        if monitor.projects and markdown_report:
        ...
        # Send email-based report if emails are configured
        if monitor.emails and all_email_categorized and email_report_saved:
            logger.info("Sending email notification for email-based report...")
            
            # Generate email subject for email-based report
            email_addresses = ', '.join([e['email'].split('@')[0] for e in monitor.emails])
            email_subject = f"📧 Gerrit Activity Report by Owner - {email_addresses} - {report_date}"
            
            # Convert markdown to HTML
            email_html_content = email_notifier.markdown_to_html(email_markdown_report)
            
            email_based_success = email_notifier.send_email(
                subject=email_subject,
                html_content=email_html_content,
                text_content=email_markdown_report
            )
            
            if email_based_success:
                logger.info("✅ Email-based report notification sent successfully!")
            else:
                logger.warning("⚠️  Failed to send email-based report notification (report still saved)")
    else:
